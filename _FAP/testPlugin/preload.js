"use strict";
console.log("Line 2 preload loaded");
/* global Pear */
...
if (process.isMainFrame) {
  ...
  if (isDecal === false) {
    ...
    window[
      Symbol.for("hypercore-id-encoding")
    ] = require("hypercore-id-encoding");
  }
  ...
  customElements.define(
    ...
    class extends HTMLElement {
      ...
      async #maclights(visible) {
        ...
        await gui.ipc.setWindowButtonPosition({
          id: gui.id,
          point: { x, y: y - 6 },
        });
      }
      ...
      async #setMaximizable(value) {
        if (!isMac) {
          this.root.querySelector("#max").style.display = value
            ? "inline"
            : "none";
        } else {
      ...
      async #setMinimizable(value) {
        if (!isMac) {
          this.root.querySelector("#min").style.display = value
            ? "inline"
            : "none";
        } else {
      ...
      constructor() {
        ...
        this.template.innerHTML = isWindows
          ? this.#win()
          : isMac
          ? this.#mac()
          : this.#gen();
        ...
        this.#onfocus = () =>
          this.root.querySelector("#ctrl").classList.add("focused");
        this.#onblur = () =>
          this.root.querySelector("#ctrl").classList.remove("focused");
        this.#demax = () =>
          this.root.querySelector("#ctrl").classList.remove("max");
      }

      async #min() {
        await Pear.Window.self.minimize();
      }
      async #max(e) {
